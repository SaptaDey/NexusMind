...
class ASRGoTGraph(TimestampedModel):
    ...
    _nx_graph: nx.MultiDiGraph = Field(default_factory=nx.MultiDiGraph, exclude=True)

    # Metadata about the graph itself, e.g., current stage, overall query
    graph_metadata: Dict[str, Any] = Field(default_factory=dict)

    @field_validator("_nx_graph", mode="before")
    @classmethod
    ...
    def remove_edge(self, edge_id: str) -> Optional[Edge]:
        ...
        if edge:
            if self._nx_graph.has_edge(edge.source_id, edge.target_id, key=edge_id):
    ...
    def add_hyperedge(self, hyperedge: Hyperedge) -> None:
        ...
        # Representation in _nx_graph for hyperedges can be tricky.
        # Common approaches:
    ...
    # but for managing the graph structure, manual updates to _nx_graph are fine.
    # We need to ensure _nx_graph is not directly part of serialization if it becomes too complex.
    # The `exclude=True` for _nx_graph handles this for default Pydantic serialization.
